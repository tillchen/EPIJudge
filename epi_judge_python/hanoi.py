...
def compute_tower_hanoi(num_rings: int) -> List[List[int]]:
    ...
    def helper(num_rings_to_move: int, from_peg: int, to_peg: int, use_peg: int):
        if num_rings_to_move == 0:
            return
        helper(num_rings_to_move - 1, from_peg, use_peg, to_peg)
        pegs[to_peg].append(pegs[from_peg].pop())
        result.append([from_peg, to_peg])
        helper(num_rings_to_move - 1, use_peg, to_peg, from_peg)
